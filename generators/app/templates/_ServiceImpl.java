...
<% if(implementsBaseService == true) { %>
  ...
  private <%= repositoryClass %> <%= repositoryClass_cc %>;

  @Autowired
  public <%= serviceClass %>ServiceImpl(<%= repositoryClass %> <%= repositoryClass_cc %>) {
    this.<%= repositoryClass_cc %> = <%= repositoryClass_cc %>;
  }
